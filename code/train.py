...
from utils.train_utils import construct_knn_graph, train_node2vec_emb
from models.bsal import BSAL, BSAL_2, BSAL_3, BSAL_4
...
def train(model, train_loader, knn_graph, device, optimizer, train_dataset):
    model.train()
    ...
    for data in tqdm(train_loader, ncols=70):
        data = data.to(device)
        optimizer.zero_grad()
        logits_1, logits_2, logits_3 = model(data, knn_graph)
...
@torch.no_grad()
def test(args, loader, knn_graph, model, device):
    model.eval()

    y_pred, y_true = [], []
    for data in tqdm(loader, ncols=70):
        data = data.to(device)
        _, _, logits = model(data, knn_graph)
...
def run(args):
    ...
    model = BSAL_4(train_dataset, train_dataset[0].num_features, hidden_channels=32, out_channels=32, num_layers=3).to(device)
    logger.info(model)
    knn_graph = knn_graph.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.wd)
    schedular = lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.8)
    # schedular = lr_scheduler.ExponentialLR(optimizer, gamma=0.8)
    ...
    for epoch in range(1, args.epochs):
        schedular.step()
        loss = train(model, train_loader, knn_graph, device, optimizer, train_dataset)
        results = test(args, val_loader, knn_graph, model, device)
        ...
        if args.metric == 'auc_ap':
            ...
            if val_auc > best_val_auc:
                ...
                best_val_ap = val_ap
                test_results = test(args, test_loader, knn_graph, model, device)
            ...
            if val_hits > best_val_hits:
                ...
                test_hits = test(args, test_loader, knn_graph, model, device)['hits@{}'.format(args.K)]
                patience = 0
            else:
                patience += 1
            if patience >= args.patience:
...
if __name__ == '__main__':
    ...
    handler = logging.FileHandler('../results/BSAL/Log_BSAL_{}_Use_feat_{}_{}.txt'.format(args.dataset.capitalize(), str(args.use_feat), exp_time))
